...
function Uberui:ADDON_LOADED()
  ...
  UberuiDB = removeoldDB(UberuiDB)

  local function initDB(def, tbl, save)
  ...
  self.db = initDB(defaults, db, UberuiDB)
  db = self.db
end


function Uberui:PLAYER_LOGOUT() 
  function saveDB(tbl, def, save)
    if type(tbl) ~= "table" then return {} end
    if type(save) ~= "table" then save = {} end
    if type(def) ~= "table" then return {} end
  ...
  UberuiDB = saveDB(db, defaults, UberuiDB)
end
...
function Uberui:ShowGryphons()
  if self.db.general.gryphon then
